...
async function uploadExcelToDatabase(filePath: string, tableID: string) {
  try {
    ...
    const insertQuery = `INSERT INTO t_${tableID} (${columns}) VALUES (${placeholders})`;
...
const excelPluginUpload: FastifyPluginCallback = (fastify, options, done) => {
  ...
  fastify.post<{ Params: RequestParams }>('/upload-excel/:tableID', async (request: FastifyRequest<{ Params: RequestParams }>, reply) => {
